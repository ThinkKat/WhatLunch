...
async def fetch_auction_list(url: str, /, no_cars: int | None = None) -> list:
    ...
    if len(tabmenu) == 5:
        ...
        while True:
            try:
                await tab.goto(auction_url, wait_until="domcontentloaded")
            except TimeoutError:
                print("Retry after timeout")
                continue
            except Error:
...
if __name__ == "__main__":
    async def main():
        ...
        with open(f"automart/daily/closed/auction_url_check_list.json", "w", encoding="utf-8") as f:
            auction_url_check_list = {url:0 for url in auction_url_list}
