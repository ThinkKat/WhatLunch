...
def lambda_handler(event, context):
    ...
    try:
        ...
        print(f"[autoinside] 처리 시작: s3://{source_bucket}/{source_key}")
        ...
        for row in csv_reader:
            if row[col_indices["경매상태"]] == "낙찰":
                new_row = [
                    ...
                    row[col_indices["색상"]],
                    to_int(row[col_indices["낙찰가"]]),
                    auction_house,
                    row[col_indices["경매종료일"]],
                ]
                processed_rows.append(new_row)

        if not processed_rows:
            print("[autoinside] 처리할 낙찰 데이터가 없습니다.")
        ...
        print(f"[autoinside] 처리 완료: s3://{source_bucket}/{target_key}")
        return {
            "statusCode": 200,
